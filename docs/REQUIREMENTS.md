...
| 5.7 | Once all 3 rubbers are in, either captain can still edit for 7 days, then admin-only | 🔁 **replaced** | The 7-day window was removed in favor of a user-directed model: `fixtures.finalized_at` is stamped by a database trigger (`0014`/`0015_ratings_optional.sql`) the moment all 3 rubbers are scored — at that instant, not after a delay, captains lose edit rights on scores. Admin can still edit scores after finalization. Strategy Builder ratings (15 below) are deliberately **not** part of this condition — they're optional and never locked, on either side of finalization. |
| — | A running tie score (rubbers won by each side) and each rubber can be entered in any order | 🔁 (added) | `TieScoreTally` in `ScoreEntryPage.jsx`, computed live off the same `rubbers` rows every save updates — not in the original numbered spec, added on request. Standings (Section 6) update the same way, live, wherever they're shown. |
